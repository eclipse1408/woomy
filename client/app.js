...
let app = function(modules) {
   ...
   color = 
    ...
    let mixColors = function() {
        ...
        return function(c1, c2, weight=.5) {
            ...
            for (var hex = "#", i = 1; i <= 6; i += 2) {
                for (var v1 = h2d(c2.substr(i, 2)), v2 = h2d(c1.substr(i, 2)), val = Math.floor(v2 + (v1 - v2) * weight).toString(16); val.length < 2; )
                    val = "0" + val;
                hex += val;
            }
            return hex;
    ...
    function setColors(context, givenColor) {
        config.graphical.neon ? (context.fillStyle = getColorDark(givenColor),
        context.strokeStyle = givenColor) : (context.fillStyle = givenColor,
   ...
   /*start of mockups*/ let mockups = [] /*end of mockups*/
        ...
        skill: function() {
            ...
            return {
                ...
                update: function() {
                    ...
                    score.get() - deduction >= levelScore && (deduction += levelScore,
                    level += 1);
                },
    ...
    window.onload = function() {
        ...
        document.onkeydown = function(e) {
            ...
            if (!global.disableEnter && key === global.KEY_ENTER && !global.gameStart)
                startGame();
            if (global.diedAt + 3e3 - Date.now() <= 0 && global.resetMenuColor && key === global.KEY_ENTER)
                setTimeout(function() {
    ...
    let socketInit = function() {
                    ...
                    const process = function() {
                        ...
                        return function() {
                            ...
                            if (type & 0x01) {
                                ...
                                z.layer = get.next();
                                z.color = get.next();
                                if (z.id === gui.playerid)
                                    player.color = z.color;
                                if (isNew) {
                                ...
                                if (isNew)
                                    z.render = {
                                        ...
                                        x: z.x,
                                        y: z.y,
                                        lastx: z.x - metrics.rendergap * config.roomSpeed * (100 / 3) * z.vx,
                                        // 1000 / 30
                                        lasty: z.y - metrics.rendergap * config.roomSpeed * (100 / 3) * z.vy,
                                        // 1000 / 30
                                        lastvx: z.vx,
                                        lastvy: z.vy,
                                        lastf: z.facing,
                    ...
                    return function() {
                        ...
                        for (let i = 0; i < entities.length; i++) {
                            ...
                            if (e.render.status.getFade() !== 0 && isInView(e.render.x - player.renderx, e.render.y - player.rendery, e.size, 1))
                                output.push(e);
                            else if (e.render.textobjs != null)
                        ...
                        entities.sort(function(a, b) {
                            ...
                            if (!sort)
                                sort = b.id - a.id;
                            if (!sort)
                                throw new Error('Something is up again...');
                            return sort;
                        });
                    }
                    ;
                            ...
                            case -1:
                                {
                                    ...
                                    if (index === -1)
                                        logger.warn('Remove request for a minimap node we were not aware of!');
                                    else
        ...
        return function(port) {
            ...
            window['setColor'] = function(id) {
                if (id < 0)
                    return logger.warn('Please specify a valid color ID! Note that IDs 0-31 are colors.');
                socket.talk('D', 1, id);
            ...
            window['setSkill'] = function(amount) {
                if (isNaN(amount) || amount < 0)
                    return logger.warn("Please specify a valid amount of stats! Note that the amount can't be below 0 or above 90.");
            ...
            window['setScore'] = function(score) {
                if (isNaN(score))
                    return logger.warn("Please specify a valid score!");
            ...
            window['setSize'] = function(size) {
                if (isNaN(size) || size < 0 || size > 3000)
                    return logger.warn("Please specify a valid size value!");
            ...
            window['setTank'] = function(tank) {
                if (!tank)
                    return logger.warn("Please specify a valid tank!");
                socket.talk('D', 5, tank);
                logger.info('Set your tank to ' + tank + '.');
            }
            ;
            window['setStat'] = function(stat, value) {
                if (stat !== 'weapon_speed' && stat !== 'weapon_reload' && stat !== 'move_speed' && stat !== 'max_health' && stat !== 'body_damage' && stat !== 'weapon_damage' && stat !== 'names')
                    return logger.warn('Invalid stat name! Input setStat("names") for a list of stats.');
                if (stat == 'names')
                    return logger.info('Stat Names: weapon_speed, weapon_reload, move_speed, max_health, body_damage, weapon_damage.'),
                    value = 0;
                if (isNaN(value) || (stat == 'weapon_reload' && value <= 0))
                    return logger.warn('Please specify a valid value for this stat!');
            ...
            window['spawnEntity'] = function(entity, x, y, team, color, size, value) {
                if (!entity || !isNaN(entity))
                    return logger.warn("Please specify a valid entity!");
                if (!x || !y || (isNaN(x) && x !== 'me' || isNaN(y) && y !== 'me'))
                    return logger.warn("Please specify a valid (X,Y) position!");
                if (!team || (isNaN(team) && team !== 'me'))
                    return logger.warn("Please specify a valid team!");
                if (color < 0 || (isNaN(color) && color !== 'rainbow' && color !== 'default'))
                    return logger.warn("Please specify a valid color ID!");
            ...
            window['setChildren'] = function(amount) {
                if (!amount || amount < 0 || isNaN(amount))
                    return logger.warn('Please specify a valid maxChildren value!');
            ...
            window['setInvisible'] = function(fadeOut, fadeIn, maxFade) {
                ...
                if (valueCheck(fadeOut) || valueCheck(fadeIn) || valueCheck(maxFade))
                    return logger.warn('Please specify a valid invisibility alpha value!');
            ...
            window['setFOV'] = function(fov) {
                if (!fov || fov < 0 || fov > 500 || isNaN(fov))
                    return logger.warn('Please specify a valid FoV value!');
            ...
            window['setSpinSpeed'] = function(speed) {
                if (!speed || isNaN(speed))
                    return logger.warn('Please specify a valid speed value!');
            ...
            window['setTeam'] = function(teamID) {
                if (isNaN(teamID))
                    return logger.warn('Please specify a valid team ID!');
                socket.talk('D', 15, teamID);
                logger.info('Set your team ID to ' + teamID + '.');
            }
            ;
            window['skillSet'] = function(s1, s2, s3, s4, s5, s6, s7, s8, s9, s10) {
                // Simplify?
                let s = function(skill) {
                    return skill < 0 || isNaN(skill);
                };
                if (s(s1) || s(s2) || s(s3) || s(s4) || s(s5) || s(s6) || s(s7) || s(s8) || s(s9) || s(s10))
                    return logger.warn('Please specify a valid skill-set array!');
            ...
            window['rainbowSpeed'] = function(speed) {
                if (isNaN(speed))
                    return logger.warn('Please specify a valid rainbow speed value!');
            ...
            window['setControl'] = function(amount) {
                if (isNaN(amount) || amount < 0)
                    return logger.warn('Please specify a valid amount of entities to control!');
                ...
                case 'u':
                    {
                        ...
                        if (cam.time > player.lastUpdate) {
                            ...
                            if (metrics.rendergap <= 0)
                                logger.warn('Yo some bullshit is up...');
                            ...
                            logger.info('This is old data! Last given time: ' + player.time + '; offered packet timestamp: ' + cam.time + '.');
                        socket.cmd.talk();
                        updateTimes++;
                    }
                    break;
...
*/
    ...
    function startGame() {
        ...
        if (!global.socket)
            global.socket = socketInit(serverPort);
    ...
    let drawEntity = function() {
        function drawPoly(context, centerX, centerY, radius, sides) {
            ...
            if (!sides)
                ...
                // long boy
                for (let[scale,theta] of [[8, 0], [1, 120], [1, 240]])
                    context.lineTo(centerX + radius * scale * Math.cos((theta * Math.PI) / 180 + angle), centerY + radius * scale * Math.sin((theta * Math.PI) / 180 + angle));
                        ...
                        for (let i = 157; i < 190; i++) if (sides === i) {
                            context.save();
                            context.translate(centerX, centerY);
                            context.rotate(angle);
         ...
         context.arc(centerX, centerY, radius, 0, 2 * Math.PI, false);
                ...
                let nedFlanders = new Image()
                  ...
                  ,  acolyte = new Image()
                  , speedy = new Image();
                nedFlanders.src = 'https://cdn.glitch.global/2303b08b-1284-4844-8328-f99b8d92e812/ned.png?v=1681765513220';
                aero.src = '';
                      acolyte.src = 'https://cdn.glitch.global/d8c30435-a1d9-4c5d-96f8-be731a3b1086/image_2023-04-22_144145046.png?v=1682188905745',
                chillHue.src = 'https://cdn.glitch.global/2303b08b-1284-4844-8328-f99b8d92e812/chillHue.png?v=1681765540568';
                poisonHue.src = 'https://cdn.glitch.global/2303b08b-1284-4844-8328-f99b8d92e812/poisonHue.png?v=1681765505924';
                ied.src = 'https://cdn.glitch.global/2303b08b-1284-4844-8328-f99b8d92e812/IED.png?v=1681765534932';
                stry.src = 'https://cdn.glitch.global/2303b08b-1284-4844-8328-f99b8d92e812/do_not_open_at_any_cost.jpg?v=1681765526293';
                hotwheels.src = 'https://cdn.glitch.global/2303b08b-1284-4844-8328-f99b8d92e812/hotwheels.png?v=1681765522044';
                omega.src = 'https://cdn.glitch.global/2303b08b-1284-4844-8328-f99b8d92e812/omega.png?v=1681765508537';
                missing.src = 'https://cdn.glitch.global/2303b08b-1284-4844-8328-f99b8d92e812/missingno.bmp?v=1681765519553';
                speedy.src = 'https://cdn.glitch.global/2303b08b-1284-4844-8328-f99b8d92e812/speedy.bmp?v=1681765501284';
                context.save();
                context.beginPath();
                context.translate(centerX, centerY);
                context.rotate(angle);
                switch (sides) {
                case 10000:
                    context.drawImage(nedFlanders, -radius * 1.5, -radius * 2.5, radius * 3, radius * 5);
                    break;
                case 10001:
                    context.drawImage(aero, -radius, -radius, radius * 2, radius * 2);
                    break;
                case 10002:
                    context.drawImage(chillHue, -radius * 4, -radius * 4, radius * 8, radius * 8);
                    break;
                case 10003:
                    context.drawImage(poisonHue, -radius * 4, -radius * 4, radius * 8, radius * 8);
                    break;
                case 10004:
                    context.drawImage(ied, -radius, -radius, radius * 2, radius * 2);
                    break;
                case 10005:
                    context.drawImage(stry, -radius, -radius, radius * 2.25, radius * 2);
                    break;
                case 10006:
                    context.drawImage(hotwheels, -radius, -radius, radius * 2, radius * 2);
                    break;
                case 10007:
                    context.drawImage(omega, -radius, -radius, radius * 2, radius * 2);
                    break;
                case 10008:
                    context.drawImage(missing, -radius, -radius * 1.5, radius * 2, radius * 3);
                    break;
                case 10009:
                    context.drawImage(speedy, -radius, -radius, radius * 2, radius * 2);
                    break;
                    case 10010:
                    context.drawImage(acolyte, -radius * 4, -radius * 4, radius * 8, radius * 8);
                    break;
...
*/
        ...
        function drawTrapezoid(context, x, y, length, height, aspect, angle, skin) {
            ...
            let r = [Math.atan2(h[0], length), Math.atan2(h[1], length)]
              , l = [Math.sqrt(length * length + h[0] * h[0]), Math.sqrt(length * length + h[1] * h[1])];
            context.beginPath();
            switch (skin) {
            case 0:
                // Normal Barrel
...
*/
            ...
            if (source.guns.length === m.guns.length) {
                ...
                for (let i = 0; i < m.guns.length; i++) {
                    ...
                    //setColors(context, mixColors(getColor(g.color), render.status.getColor(), render.status.getBlend()));
                    drawTrapezoid(context, xx + drawSize * gx, yy + drawSize * gy, drawSize * (g.length / 2 - (g.aspect === 1 ? position * 2 : 0)), drawSize * g.width / 2, g.aspect, g.angle + rot, g.skin);
                }
            } else
        ...
        if (instance.id !== gui.playerid) {
            if (instance.nameplate) {
                ...
                if (instance.name.includes('U\u2000S\u2000A')) {
                    ...
                    instance.render.textobjs[0].draw(instance.name, x, y - realSize - 30, 16, fill, 'center');
    ...
    window.requestAnimFrame = function() {
        return window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.msRequestAnimationFrame || function(callback) {/*window.setTimeout(callback, 100 / 6)*/
        }
        ;
    }();
        ...
        return function(ratio) {
            ...
            }
            {
                ...
                if (!config.graphical.screenshotMode)
                    entities.forEach(function drawHealthLoop(instance) {
                        let x = ratio * instance.render.x - px
                          , y = ratio * instance.render.y - py;
                        x += global.screenWidth / 2;
                        y += global.screenHeight / 2;
            ...
            }
            {
                ...
                do {
                    ...
                    for (let {x, y, colorIndex, index} of tiles) {
                        ...
                        if (ax < -50 || ax + size - 50 > global.screenWidth)
                            continue;
                        ctx.globalAlpha = .75;
              ...
              , max = lb.max;
            {
                if (!config.graphical.disableMessages) {
                    let vspacing = 4, height = 18, x = global.screenWidth / 2, y = 20, fill;
                ...
                gui.skills.forEach(function drawSkillBar(skill) {
                    ...
                    if (cap) {
                        ...
                        if (blocking) {
                            ...
                            for (let i = cap + 1; i < _max; i++)
                                drawGuiLine(x + (len - gap) * ska(i), y + 1.5, x + (len - gap) * ska(i), y - 3 + height);
                        }
                        ctx.strokeStyle = color.black;
                        ctx.lineWidth = 1;
                        ...
                        if (level)
                            text.skillValues[ticker - 1].draw(textcolor === col ? 'MAX' : '+' + level, Math.round(x + len + 4) + .5, y + height / 2, height - 5, col, 'left', 1);
                        y -= height + spacing;
                    }
                });
                global.clickables.hover.place(0, 0, y, .8 * len, .8 * (global.screenHeight - y));
                if (gui.points !== 0)
                    text.skillPoints.draw('x' + gui.points, Math.round(x + len - 2) + .5, Math.round(y + height - 4) + .5, 20, color.guiwhite, 'right');
            }
            {
                let spacing = 4
            ...
            }
            {
                let spacing = 4
                  ...
                  , x = global.screenWidth - len - 20
                  , y = 20 + height + 7;
                text.lbtitle.draw('Leaderboard:', Math.round(x + len / 2) + .5, Math.round(y - 6) + .5, height + 4, /*'#B6E57C'*/
                color.guiwhite, 'center');
                let i = 0;
                for (let k = 0; k < lb.data.length; k++) {
                    ...
                    if (entry.label.includes('Trans Rights'))
                        entry.image.color = 212;
                    if (entry.label.includes('4NAX'))
                        entry.image.color = 6;
            ...
            }
            {
                ...
                if (gui.upgrades.length > 0) {
                    ...
                    let spacing = 10
                      ...
                      , y2 = y
                      , ticker = 0
                      , len = alcoveSize * global.screenWidth / 2.05
                      , //100
                    height = len;
        ...
        let text = {
          ...
          , getDeath = function getDeath() {
            ...
            if (global.finalKillers.length) {
                txt = '\uD83D\uDD2A Succumbed to';
                for (let i = 0; i < global.finalKillers.length; i++)
                    txt += ' ' + util.addArticle(mockups[global.finalKillers[i]].name) + ' and';
                txt = txt.slice(0, -4) + '.';
            } else
    ...
    let gameDrawBeforeStart = function() {
        let text = {
          ...
          , tip = ['Press the E key to enable autofire.', 'Press the C key to enable autospin.', 'Press the R key to disable drone and auto turret AI.', 'Hold the N key to level up.', 'Hold the Z key to change the color of the upgrade menu.', 'Press the U key to show the class tree.', 'Hold the M key and press a number stat to instantly max out that stat.', 'Hold the L key to show extra debug stuff above the minimap.', 'If Arras is having a low frame rate, you can try enabling low graphics in the options menu.', 'The Three Upgrade Rows option may make the upgrade menu look better.', "Don't like the rounded edges look of Arras? Try the Use Miter Edges option.", 'Begging for beta-tester on this server is like digging your own grave.', 'Naming yourself "Trans Rights" makes your name have the colors of the Transgender Pride Flag.', "Don't get salty just because someone kills you; it's just a game, after all.", 'Bosses are spawned by a randomizer; they may spawn every 8 to 15 minutes.', 'The Password is 4NAX.', 'The Password is Silvy.', 'The Password is Trans Rights.', 'Sanctuaries are spawned by a randomizer; they may spawn every 3 to 6 minutes.', 'Sometimes it helps to press CTRL+F5 if your UI is bugged out.', 'Sanctuaries can be seen on the minimap, and will spawn a boss when killed.'][Math.floor(Math.random() * 19)];
        return function() {
...
, function(module, exports) {
    ...
    exports.handleLargeNumber = function(x) {
        ...
        if (x < Math.pow(10, 15))
            return (x / Math.pow(10, 12)).toFixed(2) + "t";
        if (x < Math.pow(10, 18))
            return (x / Math.pow(10, 15)).toFixed(2) + "qd";
        if (x < Math.pow(10, 21))
            return (x / Math.pow(10, 18)).toFixed(2) + "qt";
        if (x < Math.pow(10, 24))
            return (x / Math.pow(10, 21)).toFixed(2) + "sx";
        if (x < Math.pow(10, 27))
            return (x / Math.pow(10, 24)).toFixed(2) + "sp";
        if (x < Math.pow(10, 30))
            return (x / Math.pow(10, 27)).toFixed(2) + "o";
        if (x < Math.pow(10, 33))
            return (x / Math.pow(10, 30)).toFixed(2) + "n";
        if (x > 1e38)
            return "Infinity";
        if (isNaN(x))
            return "NaN";
    ...
    exports.pullJSON = function(filename) {
        let request = new XMLHttpRequest()
          , url = "./" + filename + ".json?v=b15e97d76d6f246e319ca965a098c02f735e4c32c95ccee61df0e699455e0b604126581f4cadf8c0";
        ...
        _createClass(Canvas, [{
            ...
            value: function keyboardDown(event) {
                ...
                case global.KEY_ENTER:
                    if (global.diedAt + 3e3 - Date.now() > 0 || (global.disconnected && global.gameStart))
                        return;
                    if (global.died)
                ...
                case global.KEY_LEFT_ARROW:
                    if (!global.died && global.showTree)
                        return global.scrollX -= global.scrollX <= 0 ? 0 : .0045;
                case global.KEY_LEFT:
                    this.parent.socket.cmd.set(2, 1);
                    break;
                case global.KEY_RIGHT_ARROW:
                    if (!global.died && global.showTree)
                        return global.scrollX += global.scrollX >= 1 ? 0 : .0045;
                ...
                case global.KEY_UPGRADE_COLOR:
                    if (global.died)
                        return;
                    if (global.tankMenuColor >= 185)
                        global.tankMenuColor = 100;
            ...
            value: function mouseDown(mouse) {
                ...
                case 0:
                    ...
                    if (statIndex !== -1)
                        ...
                        if (upgradeIndex !== -1)
                            this.parent.socket.talk('U', upgradeIndex);
                        else
                            this.parent.socket.cmd.set(4, 1);
                    }
                    break;
            ...
            value: function gameInput(mouse) {
                ...
                if (!this.parent.target.x)
                    this.parent.target.x = .5;
                if (!this.parent.target.y)
                    this.parent.target.y = .5;
